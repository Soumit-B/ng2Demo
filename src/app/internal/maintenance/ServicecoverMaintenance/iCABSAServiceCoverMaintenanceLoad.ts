...
export class ServiceCoverMaintenanceLoad {
    ...
    public window_onload(): void {
        ...
        // Always Attempt To Get The CallCentre CallLogID;
        this.context.riExchange.riInputElement.SetValue(this.context.uiForm, 'CallLogID',
            this.context.riExchange.getParentHTMLValue('CurrentCallLogID'));
        this.context.riExchange.riInputElement.SetValue(this.context.uiForm, 'ClosedWithChanges',
            this.context.riExchange.getParentHTMLValue('ClosedWithChanges'));
        ...
        // Create The Settings For The ProductComponent Grid (MEDICAL);
        //Call CreateComponentGrid();
        //Call CreateDisplayGrid();
        let iCounter;
        ...
        //Reference to variable definitiions removed as they are globally defined;
        ...
        this.context.riMaintenance.Execute(this.context, function (data: any): any {
            ...
            this.context.controls[400].disabled = (data['AllowUpdateInd'] === 'N');
            this.context.iCABSAServiceCoverMaintenanceLoad.window_onload_contd();
        }, 'POST');
    }

    public window_onload_contd(): void {
        this.context.riMaintenance.clear();
    ...
    public window_onload_contd1(): void {
        this.context.riMaintenance.clear();
        ...
        //************************************************************************************
        //* Only show Trial Period && Free of Charge options for CONTRACTS                  *
        //************************************************************************************
        if (this.context.riExchange.getCurrentContractType() === 'C') {
            // SRS040A Use flags to control display of trail && FOC;
        ...
        //************************************************************************************
        //* Get Parent values for ContractNumber, PremiseNumber etc.                         *
        //************************************************************************************
        if ((this.context.parentMode === 'Search'
            ...
            || this.context.parentMode === 'ContactUpdate') && this.context.riMaintenance.CurrentMode !== MntConst.eModeAdd) {
            ...
            if (this.context.parentMode !== 'ContactAdd' && this.context.riExchange.getParentHTMLValue('ProductCode')) {
                ...
                if (!this.context.riExchange.getParentHTMLValue('ProductCode')) {
                    this.context.riExchange.riInputElement.SetValue(this.context.uiForm, 'ProductCode',
                        this.context.riExchange.getParentAttributeValue('ProductCode'));
                }
                // if ( this.context.riExchange.RowSelected()) {
                //     let vntReturnData;
                //     vntReturnData = this.context.riExchange.GetReturnData();
                //     this.context.riExchange.getParentHTMLValue('ProductCode', true, vntReturnData(0));
                // }
            }
        }
        ...
        if (this.context.riExchange.getParentHTMLValue('CurrentCallLogID') !== '' &&
            ...
            this.context.riMaintenance.FunctionSelect = false;   // changed by RG;
        }
        ...
        // SH 14/12/2005 #17876 - if( parent is new //Portfolio General Maintenance// screen;
        //                        ) { don//t allow add || select as user should return;
        //                        to calling program to do these functions.;
    ...
    public window_onload_contd2(callback?: any): void {
        if (this.context.pageParams.vbEnableProductServiceType) {
            this.context.disableControl('ServiceTypeCode', true);
        } else {
            this.context.disableControl('ServiceTypeCode', false);
        }
        if (!this.context.afterSaveNavigate) {
            if ((this.context.parentMode === 'Premise-Add'
                ...
                let contractNum = this.context.getControlValue('ContractNumber');
                let contractname = this.context.getControlValue('ContractName');
                let premiseNume = this.context.getControlValue('PremiseNumber');
                let premisename = this.context.getControlValue('PremiseName');
                let contractType = this.context.getControlValue('ContractTypeCode');
                ...
                if (this.context.parentMode === 'ContactAdd') {
                    ...
                    this.context.inputParamsAccountPremise.ContractName = this.context.getControlValue('ContractName');
        ...
        this.context.riMaintenance.AddTableField('VisitCycleInWeeks', MntConst.eTypeInteger, MntConst.eFieldOptionRequried, MntConst.eFieldStateNormal, 'Required');
        ...
        //PKT - 16 SEPTEMBER 2003 - Seasonal Service Dates (Contracts Only);
        if (this.context.riExchange.getCurrentContractType() === 'C') {
        ...
        this.context.riMaintenance.AddTableField('GuaranteeCommence', MntConst.eTypeDateText, MntConst.eFieldOptionNormal, MntConst.eFieldStateReadOnly, 'ReadOnly');
        this.context.riMaintenance.AddTableFieldAlignment('GuaranteeCommence', MntConst.eAlignmentCenter);
        this.context.riMaintenance.AddTableField('GuaranteeExpiry', MntConst.eTypeDateText, MntConst.eFieldOptionNormal, MntConst.eFieldStateReadOnly, 'ReadOnly');
        ...
        // The following variable is used to identif(y the Cache that is being used to store component selections;
        ...
        this.context.riMaintenance.AddTableField('DeliveryReleaseDate', MntConst.eTypeDateText, MntConst.eFieldOptionNormal, MntConst.eFieldStateReadOnly, 'ReadOnly');
        ...
        //MSA 15/09/2004 - Changed slightly to fix error with lazy evaluation;
        if (this.context.parentMode === 'Premise-Add') {
        ...
        //15.04.2004 Added Annual Entitlement fields;
        if (this.context.pageParams.vbEnableEntitlement && this.context.riExchange.getCurrentContractType() === 'C') {
            ...
            this.context.riMaintenance.AddTableField('EntitlementAnnivDate', MntConst.eTypeDate, MntConst.eFieldOptionNormal, MntConst.eFieldStateNormal, 'Optional');  // RT 402 - now updateable;
            ...
            this.context.riMaintenance.AddTableField('EntitlementNextAnnualQuantity', MntConst.eTypeInteger, MntConst.eFieldOptionNormal, MntConst.eFieldStateNormal, 'Optional');  // #19864;
        ...
        // 05/05/05 PG SRS 41 Add Free of Charge values;
        ...
        // 13/05/2005 MSA - SRS-40 - Trial Periods;
        ...
        // 17/08/05 PG #12560 Add Start Date;
        ...
        // 10/02/06 PG SRS124 - Service Calendar;
        ...
        // 23/10/2007 MSA - Service Cover API Codes;
        ...
        // 07/11/08 #35611: UK HY - Survey Details (SIP) Don//t need syschar on this as hiding the whole tab if( OFF.;
        ...
        if (this.context.riMaintenance.CurrentMode !== MntConst.eModeSelect
            && this.context.riMaintenance.CurrentMode !== MntConst.eModeAdd) {
            this.context.riMaintenance.AddTableCommit(this.context, function (data: any): any {
                if (data && data.hasError) {
                    this.context.handleInvalidSelectionExit();
                    return;
                } else if (data && data.findResult === '<Multi>') {
    ...
    public continueTableLoad(): void {
        ...
        // SH 27/02/2006 #14352 - new parameter to return from back end;
        ...
        this.context.riMaintenance.AddTableField('InvoiceAnnivDate', MntConst.eTypeDate, MntConst.eFieldOptionNormal, MntConst.eFieldStateReadOnly, 'ReadOnly');
        ...
        this.context.riMaintenance.AddTableField('AnnualValueChange', MntConst.eTypeCurrency, MntConst.eFieldOptionNormal, MntConst.eFieldStateNormal, 'Optional');
        ...
        //MSA - Account number was!being cleared when selecting || adding && had previously selected;
        //      a Service Cover (as it is a virtual field). By adding it in here as well, &&!posting;
        //      the data through, it will get cleared when selecting || adding && will be restored when;
        //      user clicks abandon.;
        ...
        // SRS 17 Invoice Frequency select fields;
        ...
        //  this.context.riMaintenance.AddTableField('GenContractNumber',MntConst.eTypeText,MntConst.eFieldOptionNormal,MntConst.eFieldStateReadOnly,'ReadOnly');
        // SRS 29 - Seasonal Service;
        ...
        // Add Variable number of seasons to control;
        for (let i = 1; i <= 8; i++) {
        ...
        if (this.context.riMaintenance.CurrentMode !== MntConst.eModeSelect &&
            ...
            this.context.riMaintenance.AddTableCommit(this.context, function (data: any): any {
                ...
                //* Add Virtual Tables                                                               *
                //************************************************************************************
                //AIH moved to help keep the wheels of progress moving;
                if (this.context.riMaintenance.CurrentMode !== MntConst.eModeSelect) {
    ...
    public load1(): void {
        //Insert Virtual Table Descriptions;
        ...
        //************************************************************************************;
        //* Fetch Record (if( selected) && Default Mode (Add, Update etc.)                   *;
        //************************************************************************************;
        ...
        if (this.context.parentMode === 'ContactUpdate') {
            if (this.context.riExchange.getParentHTMLValue('ServiceCoverRowID') === '') {
                ...
                this.context.riMaintenance.UpdateMode();  // Go into auto update mode when running from Client Retention;
            }
        }
        ...
        //Disable Invoice Freq/Anniv Date;
        ...
        // MSA - Trial Period - Highlight the Trial Period tab - Must be done after everything;
        // } else { has been loaded, otherwise tabs get rebuilt, && selection is lost!;
        ...
        if (this.context.riExchange.riInputElement.GetValue(this.context.uiForm, 'RunningReadOnly') === 'yes') {
            this.context.serviceCoverSearchParams.showAddNew = false;
            this.context.riMaintenance.FunctionAdd = false;
            this.context.riMaintenance.FunctionSelect = false;
            this.context.riMaintenance.FunctionDelete = false;
            this.context.riMaintenance.FunctionUpdate = false;
        ...
        // if( converting a prospect from CCM ) { always allow add. Allows multiple premises to be added in the one conversion;
        ...
        this.context.riExchange.riInputElement.SetValue(this.context.uiForm, 'OriBranchServiceAreaCode',
            this.context.riExchange.riInputElement.GetValue(this.context.uiForm, 'BranchServiceAreaCode'));
        this.context.populateEllipsisParams();
        this.context.iCABSAServiceCoverMaintenance4.riExchange_CBORequest();
    }

    public getServiceCoverRowIDForCallingProgram(): void {
        //************************************************************************************;
        //* Get Rowid of ServiceCover from calling Program                                   *;
        //************************************************************************************;
        switch (this.context.parentMode) {
            // Jira iCABS-3488 : Added 'RepeatSales' to allow for a call from iCABSARRepeatSalesBranch.htm to be serviced.;
            case 'RepeatSales':
            ...
            case 'ServiceCoverGrid':
                ...
                // New mode //ServiceSuspend// to facilitate drill-down from;
                // Premise/Service Suspend Maintenance screen.;
                break;
            case 'ServiceSuspend':
